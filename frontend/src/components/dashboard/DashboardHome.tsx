...
export function DashboardHome() {
  ...
  const { data: providers } = useQuery({ queryKey: ["providers"], queryFn: () => api.get("/api/v1/llm/providers").then(r => r.data) });
  const { data: agents } = useQuery({ queryKey: ["agents"], queryFn: () => api.get("/api/v1/agents/").then(r => r.data) });
  const { data: workflows } = useQuery({ queryKey: ["workflows"], queryFn: () => api.get("/api/v1/workflows/").then(r => r.data) });
  ...
  return (
    <div className="p-8">
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4 mb-8">
        ...
        <StatCard icon={Activity} label="Platform" value={health ? "Live" : "..."} color="bg-green-600" />
      </div>
